...
/* Page fault handler.  This is a skeleton that must be filled in to implement virtual memory.  Some solutions to project 2 may also require modifying this code. At entry, the address that faulted is in CR2 (Control Register 2) and information about the fault, formatted as described in the PF_* macros in exception.h, is in F's error_code member.  The example code here shows how to parse that information.  You can find more information about both of these in the description of "Interrupt 14--Page Fault Exception (#PF)" in [IA32-v3a] section 5.15 "Exception and Interrupt Reference". */
//页面错误处理程序。这是一个骨架，必须填充它才能实现虚拟内存。项目2的一些解决方案可能还需要修改此代码。在输入时，出错的地址在CR2（控制寄存器2）中，有关错误的信息（格式如exception.h中的PF_x宏所述）在F的error_code成员中。这里的示例代码展示了如何解析这些信息。您可以在[IA32-v3a]第5.15节“异常和中断引用”中“中断14——页面错误异常（#PF）”的描述中找到这两个问题的更多信息。
static void page_fault(struct intr_frame *f) 
{
	bool not_present;  /* True: not-present page, false: writing r/o page. *///True:不存在页，false:正在写入r/o页。
	bool write;        /* True: access was write, false: access was read. *///True:访问被写入，false:访问被读取。
	bool user;         /* True: access by user, false: access by kernel. *///True：用户访问，false：内核访问。
	void *fault_addr;  /* Fault address. *///故障地址。
/* Obtain faulting address, the virtual address that was accessed to cause the fault.  It may point to code or to data.  It is not necessarily the address of the instruction that caused the fault (that's f->eip). See [IA32-v2a] "MOV--Move to/from Control Registers" and [IA32-v3a] 5.15 "Interrupt 14--Page Fault Exception (#PF)". */
//获取错误地址，即访问导致错误的虚拟地址。它可能指向代码或数据。导致错误的不一定是指令的地址（即f->eip）。参见[IA32-v2a]“MOV——移动到/移出控制寄存器”和[IA32-v3a]5.15“中断14——页面错误异常（#PF）”。
